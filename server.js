...
app.put("/mockdata/:id", (req, res) => {
  const Data = mockdata.find((b) => b.id === parseInt(req.params.id)); // to find the book by id
  if (!mockdata) return res.status(404).json({ message: "Data not found" }); // to send a 404 status code and a message if the book is not found
...
app.delete("/mockdata/:id", (req, res) => {
  const index = Data.findIndex((b) => b.id === parseInt(req.params.id)); // to find the index of the book by id
  if (index ===D-1) return res.status(404).json({ message: "Data not found" }); // to send a 404 status code and a message if the book is not found

  Data.splice(index, 1); // to delete the book from the books array
